...
object TransferFraming {
    ...
    fun writeHeader(out: OutputStream, header: FileHeader) {
        val body = FileHeaderCodec.encode(header)
        out.writeInt32(body.size)
        out.write(body)
    }

    /** @throws TransferError.MalformedStream on a bad length prefix or truncated header. */
    fun readHeader(input: InputStream): FileHeader {
        ...
        return FileHeaderCodec.decode(input.readFully(length))
    }
